...
public class WandCommand extends ACCommandBase {
    private static final String[] BOUNDS = new String[]{"x0", "y0", "z0", "x1", "y1", "z1"};
    private static final String[] COORDINATES = new String[]{"~", "0", "1"};
    private static final String[] DIRECTIONS = new String[]{"me", "up", "down", "north", "east", "south", "west", "x",
            "y", "z", "+x", "+y", "+z", "-x", "-y", "-z"};
    ...
    private void execute_set(String[] args, EntityPlayerMP player, ICommandSender sender) throws CommandException {
        ...
        if (args.length == 3) {
            ...
            if (varStr.equalsIgnoreCase("x0")) {
                ix = (int) parseDouble(sender.getPosition().getX(), varStr, false);
            } else if (varStr.equalsIgnoreCase("y0")) {
                iy = (int) parseDouble(sender.getPosition().getY(), varStr, false);
            } else if (varStr.equalsIgnoreCase("z0")) {
                iz = (int) parseDouble(sender.getPosition().getZ(), varStr, false);
            } else if (varStr.equalsIgnoreCase("x1")) {
                ax = (int) parseDouble(sender.getPosition().getX(), varStr, false);
            } else if (varStr.equalsIgnoreCase("y1")) {
                ay = (int) parseDouble(sender.getPosition().getY(), varStr, false);
            } else if (varStr.equalsIgnoreCase("z1")) {
                az = (int) parseDouble(sender.getPosition().getZ(), varStr, false);
            } else {
                throw new WrongUsageException("Wrong parameter 'bound'! /ac_wand set <bound> <value>");
            }
            ...
            int ix = (int) parseDouble(sender.getPosition().getX(), args[1], false);// bounds[0];
            int iy = (int) parseDouble(sender.getPosition().getY(), args[2], false);
            ;// bounds[1];
            int iz = (int) parseDouble(sender.getPosition().getZ(), args[3], false);
            ;// bounds[2];
            int ax = (int) parseDouble(sender.getPosition().getX(), args[4], false);
            ;// bounds[3];
            int ay = (int) parseDouble(sender.getPosition().getY(), args[5], false);
            ;// bounds[4];
            int az = (int) parseDouble(sender.getPosition().getZ(), args[6], false);
            ;// bounds[5];

            WandItem.setBounds(player, ix, iy, iz, ax, ay, az);
        } else {
            throw new WrongUsageException("Missing parameters! /ac_wand set < ? >");
        }
    }
    ...
    private void execute_expand(String[] args, EntityPlayerMP player, ICommandSender sender) throws CommandException {
        if (args.length == 2) {
            ...
            throw new WrongUsageException("Missing parameters! /ac_wand expand <val>");
        }
    }
    ...
    private void execute_expand_v(String[] args, EntityPlayerMP player, ICommandSender sender) throws CommandException {
        if (args.length == 2) {
            int value = parseInt(args[1], 0, 128);
    ...
    private void execute_expand_to(String[] args, EntityPlayerMP player, ICommandSender sender)
        ...
        if (args.length == 4) {
            ...
            int posX = (int) parseDouble(sender.getPosition().getX(), args[1], false);
            int posY = (int) parseDouble(sender.getPosition().getY(), args[2], false);
            int posZ = (int) parseDouble(sender.getPosition().getZ(), args[3], false);
            ...
            throw new WrongUsageException("Missing parameters! /ac_wand expand_to <x> <y> <z>");
        }
    }

    private void execute_expand_into(String[] args, EntityPlayerMP player, ICommandSender sender)
            throws CommandException {
        if (args.length == 3) {
            int[] bounds = WandItem.getBoundsFromPlayerOrNull(player);
            if (bounds == null) {
            ...
            int value = parseInt(args[2], 1, 128);
            ...
            switch (direction) {
                ...
                case 5:
                    ...
                    break; // -x
                default:
                    throw new WrongUsageException("Direction unknown: " + args[2]);
            }

            WandItem.setBounds(player, ix, iy, iz, ax, ay, az);
        } else {
            throw new WrongUsageException("Missing parameters! /ac_wand expand <direction> <value>");
        }
    }
    ...
    private void execute_region(String[] args, EntityPlayerMP player, ICommandSender sender) throws CommandException {
        if (args.length >= 2) {
            ...
            if (args[1].equals("butcher")) {
                ...
                AxisAlignedBB aabb = new AxisAlignedBB(bounds[0], bounds[1], bounds[2], bounds[3], bounds[4],
                        bounds[5]);
            ...
            throw new WrongUsageException("Missing parameters! /ac_wand region ...?");
        }

    }

    /****/
    private void execute_region_erase(String[] args, EntityPlayerMP player, ICommandSender sender)
            throws CommandException {
        int[] bounds = WandItem.getBoundsFromPlayerOrNull(player);
        if (bounds == null) {
            throw new WrongUsageException("You didn't select a region with your wand.");
    ...
    /****/
    private void execute_region_copy(String[] args, EntityPlayerMP player, ICommandSender sender)
            throws CommandException {
        if (args.length > 2) {
            String name = args[2];
            ...
            if (item != null) {
                ...
                player.sendMessage(new TextComponentString(
                        TextFormatting.GREEN + "Copied region to clipboard as '" + name + "'!"));
            }
        } else {
            ...
            AdventureCraft.network.sendTo(new StringNBTCommandPacket("item.copy.trigger"), player);
        }
    }

    /****/
    private void execute_region_paste(String[] args, EntityPlayerMP player, ICommandSender sender)
            throws CommandException {
        if (args.length == 2) {
            int[] bounds = WandItem.getBoundsFromPlayerOrNull(player);
            if (bounds == null) {
            ...
            if (item != null) {
                ClipboardItem.pasteRegion(item, new BlockPos(bounds[0], bounds[1], bounds[2]), player.getEntityWorld(),
                        player);
                player.sendMessage(
                        new TextComponentString(TextFormatting.GREEN + "Copied region to world: '" + name + "'."));
            } else {
        ...
        if (args.length > 2) {
            ...
            throw new WrongUsageException("Missing parameters! /ac_wand region paste <name>");
        }
    }

    /****/
    private void execute_region_trigger(String[] args, EntityPlayerMP player, ICommandSender sender)
            throws CommandException {
        int[] bounds = WandItem.getBoundsFromPlayerOrNull(player);
        if (bounds == null) {
            throw new WrongUsageException("You didn't select a region with your wand.");
    ...
    /****/
    private void execute_region_fill(String[] args, EntityPlayerMP player, ICommandSender sender)
            throws CommandException {
        if (args.length == 3) {
            ...
            throw new WrongUsageException("Missing parameters! /ac_wand region fill <block>");
        }
    }

    /****/
    private void execute_region_replace(String[] args, EntityPlayerMP player, ICommandSender sender)
            throws CommandException {
        if (args.length == 4) {
            IBlockState replace = GObjectTypeHelper.findBlockState(args[2]);
    ...
    /****/
    private void execute_region_room(String[] args, EntityPlayerMP player, ICommandSender sender)
            throws CommandException {
        if (args.length == 4) {
            ...
            { // Parse Wall Flags
                if (containsCharFlagIgnoreCase(flagsStr, 'U'))
                    f_up = true;
                if (containsCharFlagIgnoreCase(flagsStr, 'D'))
                    f_down = true;
                if (containsCharFlagIgnoreCase(flagsStr, 'N'))
                    f_north = true;
                if (containsCharFlagIgnoreCase(flagsStr, 'E'))
                    f_east = true;
                if (containsCharFlagIgnoreCase(flagsStr, 'S'))
                    f_south = true;
                if (containsCharFlagIgnoreCase(flagsStr, 'W'))
                    f_west = true;
            ...
            if (f_down)
                WorldHelper.fill(player.getEntityWorld(), ix, iy, iz, ax, iy, az, replace);
            if (f_up)
                WorldHelper.fill(player.getEntityWorld(), ix, ay, iz, ax, ay, az, replace);
            if (f_north)
                WorldHelper.fill(player.getEntityWorld(), ix, iy, iz, ax, ay, iz, replace);
            if (f_east)
                WorldHelper.fill(player.getEntityWorld(), ax, iy, iz, ax, ay, az, replace);
            if (f_south)
                WorldHelper.fill(player.getEntityWorld(), ix, iy, az, ax, ay, az, replace);
            if (f_west)
                WorldHelper.fill(player.getEntityWorld(), ix, iy, iz, ix, ay, az, replace);

            // WorldHelper.fill(player.getEntityWorld(), bounds, replace);
            return;
        } else {
            throw new WrongUsageException("Missing parameters! /ac_wand region room <UDNESW BA> <block>");
        }
    }

    /****/
    private void execute_region_repeat(String[] args, EntityPlayerMP player, ICommandSender sender)
            throws CommandException {
        if (args.length >= 3) {
            ...
            switch (direction) {
                ...
                break;
                default:
                    throw new CommandException("Direction unknown: " + directionStr);
            }
            ...
            throw new WrongUsageException("Missing parameters! /ac_wand region repeat <count> [direction]");
        }
    }
    ...
    public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args, BlockPos pos) {
        if (args.length <= 1) {
            return getListOfStringsMatchingLastWord(args, new String[]{"region", "expand", "expand_h", "expand_v",
                    "expand_to", "expand_into", ".", "..", ".o", ".c", "set"});
        }
        ...
        if (args[0].equals("region")) {
            if (args.length == 2) {
                return getListOfStringsMatchingLastWord(args, new String[]{"erase", "fill", "replace", "room",
                        "repeat", "copy", "paste", "trigger", "butcher"});
            }
