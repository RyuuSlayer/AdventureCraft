...
import de.ryuu.adventurecraft.tileentity.LockedDoorTileEntity.DoorCorner;
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand,
        ...
        if (playerIn.isSneaking() && playerIn.isCreative() && !world.isRemote) {
            if (corner == DoorCorner.BottomLeftX) {
                ((LockedDoorTileEntity) Objects.requireNonNull(world.getTileEntity(pos.add(1, 0, 0)))).toggleKey();
                ((LockedDoorTileEntity) Objects.requireNonNull(world.getTileEntity(pos.add(0, 1, 0)))).toggleKey();
                ((LockedDoorTileEntity) Objects.requireNonNull(world.getTileEntity(pos.add(1, 1, 0)))).toggleKey();
                ((LockedDoorTileEntity) Objects.requireNonNull(world.getTileEntity(pos))).toggleKey();
            }
            ...
            if (corner == DoorCorner.BottomLeftZ) {
                ((LockedDoorTileEntity) Objects.requireNonNull(world.getTileEntity(pos.add(0, 0, 1)))).toggleKey();
                ((LockedDoorTileEntity) Objects.requireNonNull(world.getTileEntity(pos.add(0, 1, 0)))).toggleKey();
                ((LockedDoorTileEntity) Objects.requireNonNull(world.getTileEntity(pos.add(0, 1, 1)))).toggleKey();
                ((LockedDoorTileEntity) Objects.requireNonNull(world.getTileEntity(pos))).toggleKey();
            }
            ...
            String keyName = Objects.requireNonNull(tehere).useSilverKey ? TextFormatting.DARK_GRAY + "silver " : TextFormatting.GOLD + "gold ";
