...
public abstract class ACGunItem extends ACWeaponItem {
    ...
    public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
        ...
        if (!tag.hasKey("reloading"))
            tag.setLong("reloading", world.getTotalWorldTime());
        ...
        if (stack.getItemDamage() >= stack.getMaxDamage()) {
            ...
            world.playSound(null, player.getPosition(), AdventureCraftSounds.DryFire, SoundCategory.AMBIENT, 3F, 1F);
            return ActionResult.newResult(EnumActionResult.FAIL, stack);
        }
        if (!tag.hasKey("fireSpeed"))
            tag.setLong("fireSpeed", world.getTotalWorldTime());
        if (!tag.hasKey("usingDone"))
            tag.setBoolean("usingDone", true);
        if (isPistol() ? tag.getBoolean("usingDone")
                : world.getTotalWorldTime() - tag.getLong("fireSpeed") >= fireSpeed()) {
    ...
    public SoundEvent reloadSound() {
        return AdventureCraftSounds.Reload;
    }

    protected abstract ACGunClipItem getClip();
