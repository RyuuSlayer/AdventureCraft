...
import de.ryuu.adventurecraft.network.packets.VoxelatorPacket;
import de.ryuu.adventurecraft.voxelator.BrushParameter;
import de.ryuu.adventurecraft.voxelator.BrushParameter.BPType;
import de.ryuu.adventurecraft.voxelator.Voxelator;
import de.ryuu.adventurecraft.voxelator.Voxelator.ActionFactory;
import de.ryuu.adventurecraft.voxelator.Voxelator.FilterFactory;
import de.ryuu.adventurecraft.voxelator.Voxelator.ShapeFactory;
import de.ryuu.adventurecraft.voxelator.actions.VXActionReplace;
import de.ryuu.adventurecraft.voxelator.params.ListBrushParameter;
import de.ryuu.adventurecraft.voxelator.predicates.VXPredicateAlways;
import de.ryuu.adventurecraft.voxelator.shapes.VXShapeSphere;
...
public class GuiVoxelator extends QADGuiScreen {
    ...
    public void applyChanges() {
        ...
        AdventureCraft.network.sendToServer(packet);
    }
    ...
    public void buildGui() {
        {
            ...
            // Header
            panel.addComponent(QADFACTORY.createLabel("Voxelator Settings", 88, 6))
                    .setFontHeight(fontRenderer.FONT_HEIGHT * 2);

            // Apply Button
            QADButton apply_button = (QADButton) QADFACTORY.createButton("Apply", 2, 0, 80).setEnabled(true)
                    .setName("apply");
    ...
    private QADRectangularComponent getCompForType(BrushParameter param, int x, int y, int width,
        ...
        if (type == BPType.LIST) {
            ...
            QADPanel panel = new QADPanel();
            if (!compound.hasKey(param.getName()))
                compound.setTag(param.getName(), new NBTTagList());
            ...
            if (lparam.getEntryType() == BPType.BLOCKSTATE) {
                ...
                for (int i = 0; i < taglist.tagCount(); i++) {
                    ...
                    QADDropdownBox box = new QADDropdownBox(new ListBlockStateListModel(sing),
                            new ListBlockStateListModel.BlockStateItem(new ItemStack(sing)));
