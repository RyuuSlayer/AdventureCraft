...
import de.ryuu.adventurecraft.network.packets.WorkbenchCraftingPacket;
                ...
                if (!container.getSlot(0).getStack().isEmpty()) {
                    if (button.id == 0) {
                        AdventureCraft.network.sendToServer(new WorkbenchCraftingPacket(
                                new ShapedRecipes(null, 3, 3, ingredients, container.getSlot(0).getStack()), true));
                    } else {
                        AdventureCraft.network.sendToServer(new WorkbenchCraftingPacket(
                                new ShapedRecipes(null, 3, 3, ingredients, container.getSlot(0).getStack()), false));
                    }
                }
            }
        } else {
