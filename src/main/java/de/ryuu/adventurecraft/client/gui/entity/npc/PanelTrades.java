...
public class PanelTrades extends NPCPanel {
    ...
    private List<NPCShop.NPCTrade> trades;
    private int index = 0;

    public PanelTrades(NPCData data, int width, int height) {
        super(data, width, height);
        trades = new ArrayList<NPCShop.NPCTrade>(data.getShop().getTrades());
    ...
    public QADPanel getPanel() {
        ...
        addtrade.setAction(new Runnable() {
            @Override
            public void run() {
                trades.add(new NPCShop.NPCTrade(new ItemStack(Items.DIAMOND), new ItemStack(Items.APPLE)));
                resetPanel();
            }
        });
