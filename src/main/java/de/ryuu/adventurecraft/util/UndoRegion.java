...
public class UndoRegion {
    ...
    public UndoRegion(BlockRegion region, World world) {
        region = new BlockRegion(region.getMinX() - 1, region.getMinY() - 1, region.getMinZ() - 1, region.getMaxX() + 1,
                region.getMaxY() + 1, region.getMaxZ() + 1);
        this.region = region;
        xPos = region.getMinX();
        yPos = region.getMinY();
        zPos = region.getMinZ();
    ...
    public static UndoRegion fromNBT(NBTTagCompound tag) {
        UndoRegion ureg = new UndoRegion(tag.getInteger("x"), tag.getInteger("y"), tag.getInteger("z"),
                tag.getInteger("width"), tag.getInteger("height"), tag.getInteger("length"),
                DimensionManager.getWorld(tag.getInteger("world")));
    ...
    private void runIterator() {
        ...
        for (BlockPos pos : BlockPos.getAllInBox(new BlockPos(region.getMinX(), region.getMinY(), region.getMinZ()),
                new BlockPos(region.getMaxX(), region.getMaxY(), region.getMaxZ()))) {
