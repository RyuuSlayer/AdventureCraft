...
public class BombExplosion { // The same thing as a normal explosion, except it doesn't destroy all blocks
    ...
    public BombExplosion(World worldIn, Entity entityIn, double x, double y, double z) {
        ...
        this.explosion = new Explosion(worldIn, entityIn, x, y, z, explosionSize, false, true);
    }
    ...
    public void doExplosionA() {
        ...
        for (Entity value : list) {
            ...
            if (!value.isImmuneToExplosions()) {
                double d12 = value.getDistance(this.explosionX, this.explosionY, this.explosionZ) / (double) f3;

                if (d12 <= 1.0D) {
                    double d5 = value.posX - this.explosionX;
                    double d7 = value.posY + (double) value.getEyeHeight() - this.explosionY;
                    ...
                    if (d13 != 0.0D) {
                        ...
                        d7 = d7 / d13;
                        d9 = d9 / d13;
                        double d14 = this.worldObj.getBlockDensity(vec3d, value.getEntityBoundingBox());
                        double d10 = (1.0D - d12) * d14;
                        value.attackEntityFrom(DamageSource.causeExplosionDamage(explosion), 7.5f);
                        double d11 = 1.0D;

                        if (value instanceof EntityLivingBase) {
                            d11 = EnchantmentProtection.getBlastDamageReduction((EntityLivingBase) value, d10);
                        }
                        ...
                        if (value instanceof EntityPlayer) {
                            EntityPlayer entityplayer = (EntityPlayer) value;
        ...
        for (int sx = -1; sx <= 1; sx++) {
            for (int sy = -1; sy <= 1; sy++) {
                for (int sz = -1; sz <= 1; sz++) {
                    ...
                    if (state.getBlock() == Blocks.STONEBRICK && state.getValue(BlockStoneBrick.VARIANT) == EnumType.CRACKED) {
                        origBlocks.add(pos);
                    }
                }
            }
    ...
    public void doExplosionB(boolean spawnParticles) {
        this.worldObj.playSound(null, this.explosionX, this.explosionY, this.explosionZ,
                SoundEvents.ENTITY_GENERIC_EXPLODE, SoundCategory.AMBIENT, 4.0F,
