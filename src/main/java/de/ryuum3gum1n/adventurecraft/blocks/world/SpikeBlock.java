package de.ryuum3gum1n.adventurecraft.blocks.world;
...
public class SpikeBlock extends ACWorldBlock implements ACITriggerableBlock {
	...
	public SpikeBlock() {
		...
		this.setDefaultState(
				this.blockState.getBaseState().withProperty(FACING, EnumFacing.UP).withProperty(ACTIVE, true));
	}
	...
	public IBlockState getStateForPlacement(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY,
		...
		try {
			return super.getStateForPlacement(worldIn, pos, facing, hitX, hitY, hitZ, meta, placer)
					.withProperty(FACING, enumfacing).withProperty(ACTIVE, true);
		} catch (IllegalArgumentException var11) {
			if (!worldIn.isRemote) {
				LOGGER.warn(String.format(
						"Invalid spike block data @ " + pos.toString() + " in " + worldIn.provider.getDimension()));
			...
			return super.getStateForPlacement(worldIn, pos, facing, hitX, hitY, hitZ, 0, placer)
					.withProperty(FACING, EnumFacing.UP).withProperty(ACTIVE, true);
		}
	}
