...
public class WandCommand extends ACCommandBase {
	...
	private void execute_set(String[] args, EntityPlayerMP player, ICommandSender sender) throws CommandException {
		...
		if (args.length == 3) {
			...
			int iy = (int) CommandBase.parseDouble(sender.getPosition().getY(), args[2], false);
			;// bounds[1];
			int iz = (int) CommandBase.parseDouble(sender.getPosition().getZ(), args[3], false);
			;// bounds[2];
			int ax = (int) CommandBase.parseDouble(sender.getPosition().getX(), args[4], false);
			;// bounds[3];
			int ay = (int) CommandBase.parseDouble(sender.getPosition().getY(), args[5], false);
			;// bounds[4];
			int az = (int) CommandBase.parseDouble(sender.getPosition().getZ(), args[6], false);
			;// bounds[5];
