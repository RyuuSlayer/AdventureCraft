...
public abstract class ACCommandBase extends CommandBase {
	...
	/*
	 * UP: 0; +y DOWN: 1; -y NORTH: 2; -z EAST: 3; +x SOUTH: 4; +z WEST: 5; -x
	 */
	public static final int parseDirection(String directionStr, EntityPlayer player) throws CommandException {
		int direction = -1;

		if (directionStr.equalsIgnoreCase("east") || directionStr.equalsIgnoreCase("+x")
				|| directionStr.equalsIgnoreCase("x"))
			direction = 3; // +x
		if (directionStr.equalsIgnoreCase("up") || directionStr.equalsIgnoreCase("+y")
				|| directionStr.equalsIgnoreCase("y"))
			direction = 0; // +y
		if (directionStr.equalsIgnoreCase("south") || directionStr.equalsIgnoreCase("+z")
				|| directionStr.equalsIgnoreCase("z"))
			direction = 4; // +z

		if (directionStr.equalsIgnoreCase("down") || directionStr.equalsIgnoreCase("-y"))
			direction = 1; // -y
		if (directionStr.equalsIgnoreCase("north") || directionStr.equalsIgnoreCase("-z"))
			direction = 2; // -z
		if (directionStr.equalsIgnoreCase("west") || directionStr.equalsIgnoreCase("-x"))
			direction = 5; // -x
		...
		if (directionStr.equalsIgnoreCase("me")) {
			...
			case WEST:
				...
				break; // -x
			default:
				throw new WrongUsageException("WHAT THE FUCK?!");
			}
		}
