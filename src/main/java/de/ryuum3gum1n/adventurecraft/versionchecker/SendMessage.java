			...
			if(latest.isGreaterVersion(current)){
				...
				cproxy.sheduleClientTickTask(new Runnable(){
					...
					public void run() {
						...
						String message = TextFormatting.YELLOW + "AdventureCraft version is outdated! Your version is " + TextFormatting.GOLD + current + TextFormatting.YELLOW + ". The latest is " + TextFormatting.GOLD + latest.getVersion() + TextFormatting.YELLOW + ".";
