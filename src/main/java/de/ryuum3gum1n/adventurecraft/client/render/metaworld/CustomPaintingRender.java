package de.ryuum3gum1n.adventurecraft.client.render.metaworld;
	...
	public void render(Item item, ItemStack stack, Tessellator tessellator, BufferBuilder buffer, double partialTick,
		...
		if (result.typeOfHit == Type.BLOCK) {
			...
			if (painting.sizeX == 16) {
				if (facing.getOpposite() == EnumFacing.EAST)
					zMove -= 1;
				if (facing.getOpposite() == EnumFacing.NORTH)
					xMove -= 1;
			}
			double yMove = (painting.sizeY == 16 || painting.sizeY == 16 * 3 ? 0 : 0.5);
			RENDER.doRender(ent, ent.posX + facing.getFrontOffsetX() * 2 + xMove, ent.posY + yMove,
					ent.posZ + facing.getFrontOffsetZ() * 2 + zMove, ent.rotationYaw, (float) partialTick);
		}
	}
