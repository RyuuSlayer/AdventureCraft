...
import de.ryuum3gum1n.adventurecraft.client.gui.qad.model.DefaultTickBoxModel;
...
public class GuiImageHologramBlock extends QADGuiScreen {
	...
	public void buildGui() {
		...
		addComponent(new QADLabel(
				"Image Hologram Block @ " + position.getX() + " " + position.getY() + " " + position.getZ(), 2, 2));
		...
		final QADTextField texturePathTextField = QADFACTORY.createTextField(tileEntity.getTextureLocation(), colp[1],
				rowp[0] + tfc, colw[1] + colw[2] + colw[3] + 6);
		texturePathTextField.setTooltip("The path of the texture to display.");
		addComponent(texturePathTextField);

		final QADTextField offsetXTextField = QADFACTORY.createTextField(tileEntity.getHologramOffsetX(), colp[1],
				rowp[3] + tfc, colw[1]);
		final QADTextField offsetYTextField = QADFACTORY.createTextField(tileEntity.getHologramOffsetY(), colp[2],
				rowp[3] + tfc, colw[2]);
		final QADTextField offsetZTextField = QADFACTORY.createTextField(tileEntity.getHologramOffsetZ(), colp[3],
				rowp[3] + tfc, colw[3]);
		...
		final QADTickBox offsetRelativeTickbox = new QADTickBox(colp[4], rowp[3] + tfc,
				new DefaultTickBoxModel(tileEntity.getHologramOffsetRelative()));
		...
		final QADTextField rotationPitchTextField = QADFACTORY.createTextField(tileEntity.getHologramPitch(), colp[1],
				rowp[4] + tfc, colw[1]);
		final QADTextField rotationYawTextField = QADFACTORY.createTextField(tileEntity.getHologramYaw(), colp[2],
				rowp[4] + tfc, colw[2]);
		...
		final QADTextField textureWidthTextField = QADFACTORY.createTextField(tileEntity.getHologramWidth(), colp[1],
				rowp[5] + tfc, colw[1]);
		final QADTextField textureHeightTextField = QADFACTORY.createTextField(tileEntity.getHologramHeight(), colp[2],
				rowp[5] + tfc, colw[2]);
		textureWidthTextField.setTooltip("Width of the hologram.",
				"If negative, texture is tiled over surface horizontally.");
		textureHeightTextField.setTooltip("Height of the hologram.",
				"If negative, texture is tiled over surface vertically.");
		addComponent(textureWidthTextField);
		addComponent(textureHeightTextField);

		final QADTextField textureUscaleTextField = QADFACTORY.createTextField(tileEntity.getHologramUscale(), colp[1],
				rowp[6] + tfc, colw[1]);
		final QADTextField textureVscaleTextField = QADFACTORY.createTextField(tileEntity.getHologramVscale(), colp[2],
				rowp[6] + tfc, colw[2]);
		...
		final QADTextField colorTextField = QADFACTORY.createTextField(tileEntity.getHologramColor(), colp[1],
				rowp[7] + tfc, colw[1]);
		colorTextField.setTooltip("Color of the hologram as hexadecimal ARGB integer.", "Example: FFFFFFFF = White",
		...
		applyButton.setAction(new Runnable() {
			...
			public void run() {
				...
				commandComp.setFloat("var_offsetX",
						parseFloat(offsetXTextField.getText(), tileEntity.getHologramOffsetX(), -128, +128));
				commandComp.setFloat("var_offsetY",
						parseFloat(offsetYTextField.getText(), tileEntity.getHologramOffsetY(), -128, +128));
				commandComp.setFloat("var_offsetZ",
						parseFloat(offsetZTextField.getText(), tileEntity.getHologramOffsetZ(), -128, +128));
				commandComp.setBoolean("var_offsetRelative", offsetRelativeTickbox.getState());

				commandComp.setFloat("var_pitch",
						parseFloat(rotationPitchTextField.getText(), tileEntity.getHologramPitch(), -360, +360));
				commandComp.setFloat("var_yaw",
						parseFloat(rotationYawTextField.getText(), tileEntity.getHologramYaw(), -360, +360));

				commandComp.setFloat("var_width",
						parseFloat(textureWidthTextField.getText(), tileEntity.getHologramWidth(), -1000, 1000));
				commandComp.setFloat("var_height",
						parseFloat(textureHeightTextField.getText(), tileEntity.getHologramHeight(), -1000, 1000));

				commandComp.setFloat("var_uscale",
						parseFloat(textureUscaleTextField.getText(), tileEntity.getHologramUscale(), -1000, 1000));
				commandComp.setFloat("var_vscale",
						parseFloat(textureVscaleTextField.getText(), tileEntity.getHologramVscale(), -1000, 1000));

				commandComp.setInteger("var_color", parseInt(colorTextField.getText(), tileEntity.getHologramColor(),
