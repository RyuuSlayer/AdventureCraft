...
public class GuiMapListEntry implements GuiListExtended.IGuiListEntry {
    ...
    public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected, float partialTicks) {
        String s = map.name;
        if (new File("." + File.separator + "saves" + File.separator + ".AC_DOWNLOADS" + File.separator + DownloadZip.getFileName(map.dlURL)).exists()) {
            s = s + TextFormatting.GREEN + " (Already downloaded)";
        }
        String s1 = map.description.split("\n")[0];
        String s2 = "Author: " + map.author + " | Requires additional mods? " + (map.additionalModsRequired ? "Yes" : "No");
        ...
        this.client.fontRenderer.drawString(s2, x + 3, y + this.client.fontRenderer.FONT_HEIGHT + this.client.fontRenderer.FONT_HEIGHT + 3, 8421504);
    }
