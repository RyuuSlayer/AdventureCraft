...
public class PanelTrades extends NPCPanel {
	...
	public QADPanel getPanel() {
		...
		if (trades.size() > 0) {
			...
			QADDropdownBox itemSelectBuying = new QADDropdownBox(new ListItemListModel(trades.get(index).getBuying()),
					new ItemItem(trades.get(index).getBuying()));
			...
			QADNumberTextField sizeFieldBuying = new QADNumberTextField(207, 60, 30, 20,
					trades.get(index).getBuying().getCount(), NumberType.INTEGER);
			...
			if (!trades.get(index).getBuying().hasTagCompound())
				trades.get(index).getBuying().setTagCompound(new NBTTagCompound());
			...
			QADDropdownBox itemSelectSelling = new QADDropdownBox(new ListItemListModel(trades.get(index).getSelling()),
					new ItemItem(trades.get(index).getSelling()));
			...
			QADNumberTextField sizeFieldSelling = new QADNumberTextField(207, this.height / 2 + 10, 30, 20,
					trades.get(index).getSelling().getCount(), NumberType.INTEGER);
