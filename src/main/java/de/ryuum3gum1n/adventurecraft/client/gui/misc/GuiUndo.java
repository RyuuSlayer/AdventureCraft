...
import de.ryuum3gum1n.adventurecraft.client.gui.qad.QADDevider;
...
public class GuiUndo extends QADGuiScreen {
	...
	public void buildGui() {
		...
		for (int i = 0; i < list.tagCount(); i++) {
			...
			if (comp.getString("user").equals(mc.player.getName()))
				color = 0xffedd209;
				...
				if (comp.isPointInside(mouseX, mouseY)) {
					...
					GuiYesNoQuestion confirm = new GuiYesNoQuestion(this, "Are you sure you want to undo this action?",
							new Runnable() {
								...
								@Override
								public void run() {
									AdventureCraft.network.sendToServer(new UndoPacket(panelIndex));
