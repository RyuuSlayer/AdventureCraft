		...
		if(button.id == 0 || button.id == 1){
			...
			if(!empty){
				if(!container.getSlot(0).getStack().isEmpty()){
					if(button.id == 0){
						AdventureCraft.network.sendToServer(new WorkbenchCraftingPacket(new ShapedRecipes(null, 3, 3, ingredients, container.getSlot(0).getStack()), true));
					}else{
						AdventureCraft.network.sendToServer(new WorkbenchCraftingPacket(new ShapedRecipes(null, 3, 3, ingredients, container.getSlot(0).getStack()), false));
					}
				}
			}
		}else{
			AdventureCraft.network.sendToServer(new WorkbenchCraftingPacket());
		}
	}
