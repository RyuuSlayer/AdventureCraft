...
public class GuiListSaveSelectionEntry implements GuiListExtended.IGuiListEntry {
    ...
    public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected, float partialTicks) {
        String s = this.worldSummary.getDisplayName() + TextFormatting.RESET + " (Save " + (this.worldSummary.getFileName().split("@SAV")[1] + ")");
        final File worldDat = new File("./" + this.containingListSel.worldPathName + "/" + this.worldSummary.getFileName() + "/adventurecraft/info.dat");
        String s1;
        try {
                ...
                try {
                    ...
                    if (worldComp.hasKey("version")) {
                        if (worldComp.getString("version").equals("vanilla")) s2 = s2 + TextFormatting.GOLD + "Vanilla";
                            ...
                            s2 = s2 + "Adventurecraft Version: " + (Reference.MOD_VERSION.equals(worldComp.getString("version")) ? "" : TextFormatting.RED) + worldComp.getString("version");
                    } else {
    ...
    public void joinWorld() {
        ...
        if (this.worldSummary.askToOpenWorld()) {
            ...
            }, I18n.format("selectWorld.versionQuestion"), "This world was loaded in Adventurecraft version " + worldComp.getString("version") + " and loading it in this version could cause corruption!", I18n.format("selectWorld.versionJoinButton"), I18n.format("gui.cancel"), 0));
