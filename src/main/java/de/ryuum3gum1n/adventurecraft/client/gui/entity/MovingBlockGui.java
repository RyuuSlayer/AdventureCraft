package de.ryuum3gum1n.adventurecraft.client.gui.entity;
...
public class MovingBlockGui extends QADGuiScreen {
	...
	public void buildGui() {
		...
		ON_INTERACT = (QADTextField) addComponent(
				new QADTextField(5 * 2 + this.width / 5, 98 + 20 * 5, this.width / 5, 20, "")
						.setTooltip("Collision must be enabled!"));

		addComponent(new QADLabel("On Collide", 6 * 3 + this.width / 5 * 2, 88 + 20 * 5));
		ON_COLLIDE = (QADTextField) addComponent(
				new QADTextField(5 * 3 + this.width / 5 * 2, 98 + 20 * 5, this.width / 5, 20, "")
						.setTooltip("Pushable must be enabled!", "Collision must be disabled!"));
		...
		SAVE.setAction(new Runnable() {
			...
			public void run() {
				AdventureCraft.network
						.sendToServer(new CreateMovingBlockPacket(x, y, z, mc.world.provider.getDimension(), // Location
																												// to
																												// place
								STATE, // Type of block
								INVISIBLE.getState(), PUSHABLE.getState(), COLLISION.getState(), NO_GRAVITY.getState(), // Boolean
																														// settings
								MOUNT_Y_OFFSET.getValue().floatValue(), // Numbers
								new String[] { ON_TICK.getText(), ON_INTERACT.getText(), ON_COLLIDE.getText(),
										ON_DEATH.getText() } // Scripts
				));
			}
		});
		addComponent(SAVE);
