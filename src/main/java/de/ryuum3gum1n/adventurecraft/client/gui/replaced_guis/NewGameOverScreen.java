...
public class NewGameOverScreen extends GuiGameOver {
    ...
    public void confirmClicked(boolean result, int id) {
        if (result) {
            ...
            Thread t = new Thread(() -> {
                try {
                    if (AdventureCraft.lastVisitedWorld == null || (AdventureCraft.lastVisitedWorld != null && !AdventureCraft.lastVisitedWorld.getSaveHandler().getWorldDirectory().getName().equals("AC_TEST")))
                    ...
                    for (String s : savesList) {
                        ...
                        FileUtils.copyDirectoryToDirectory(new File(savesDir, s), new File(savesDir, type ? ".AC_MAPS" : ".AC_SAVES"));
                        FileUtils.deleteDirectory(new File(savesDir, s));
                    }
