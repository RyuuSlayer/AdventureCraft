...
import de.ryuum3gum1n.adventurecraft.entity.NPC.dialogue.NPCDialogue.NPCDialogueOption;
...
public class GuiNPCDialogue extends GuiScreen {
	...
	private static final ResourceLocation GUI_TEXTURE = new ResourceLocation("adventurecraft:textures/gui/npcdialogue.png");
	...
	protected void mouseReleased(int mouseX, int mouseY, int state) {
		...
		int option_y = guiTop + 156 - current.getOptions().size()*10;
		 for(NPCDialogueOption option : current.getOptions()){
	        	if(mouseX >= guiLeft + 176/3 && mouseY >= option_y){
	        		if(mouseX <= guiLeft + 176/3 + fontRenderer.getStringWidth(option.option) && mouseY <= option_y + 7){
	        			AdventureCraft.network.sendToServer(new NPCDialogueOptionPacket(uuid, current.getName(), option.option, npcid));
