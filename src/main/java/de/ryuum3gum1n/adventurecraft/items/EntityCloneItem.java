...
public class EntityCloneItem extends ACItem {
		...
		if (!attacker.getEntityWorld().isRemote) {
			if (attacker.isSneaking()) {
				...
				attacker.sendMessage(
						new TextComponentString("Copied Entity \"" + target.getName() + ChatFormatting.RESET + "\"!"));
			}
		}
		return true;
	}
	...
	private Entity spawnEntity(ItemStack stack, World world, EntityPlayer player) {
		...
		if (entity == null) {
			player.sendMessage(new TextComponentString(
					ChatFormatting.RED + "An error occured while trying to clone that entity!"));
			return null;
		}
